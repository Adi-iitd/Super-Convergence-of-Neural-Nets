...
### Weight Decay
The amount of regularization must be balanced for each dataset and architecture, and the value of weight decay is a key knob to tune regularization. This requires a grid search to determine the optimal magnitude but usually does not require to search over more than one significant figure. 
...
From the above plot we can see that weight decay of 1.8e-3 (again bisecting the exponent i.e. 10^-0.75) allows us to use much larger learning rate compared to other values and is also giving the minimum test loss.
