...
Original file is located at
    https://colab.research.google.com/drive/1zK_yofx2HVgSzW-SNS4kYpCRnvvWSBD8
"""
...
batch_size = 128; lab_bin = LabelBinarizer(); img_height = img_width = 32; img_channels = 3; 
log_path = "CIFAR10/"; colors = ['r', 'g', 'b', 'k', 'y', 'm', 'c'];

# Make the directory if it doesn't exists
if not os.path.exists(log_path): os.makedirs(log_path); 

# Load the train and test data and do the 0-1 scaling
data_ld = tf.keras.datasets.cifar10; (X_train_val, y_train_val), (X_test, y_test) = data_ld.load_data();
X_train_val = X_train_val/255.; X_test = X_test/255. 
if img_channels == 1: 
    X_train_val = np.expand_dims(X_train_val, axis = 3); X_test = np.expand_dims(X_test, axis = 3);

# Get the One-hot encoding of the target variables
...
num_train_iters = X_train.shape[0]//batch_size; num_val_iters = num_test_iters = X_test.shape[0]//batch_size;
...
#   Creating the dataset versions of the original tensors
    train_dataset = get_dataset((X_train, y_train)); 
    val_dataset = get_dataset((X_val, y_val)); 
    test_dataset = get_dataset((X_test, y_test));
  
#   Applying augmentation techniques to get the distorted version of the original dataset 
#   Randomly shuffling the training dataset, batching them and then prefetching them to make sure that a batch is ready to be served at all time   
...
#   Creating the iterators
    ...
    iterator = tf.data.Iterator.from_string_handle(handle, train_dataset.output_types, train_dataset.output_shapes); next_element = iterator.get_next()
...
def conv_2d(x, kernel_size = 3, stride = 1, out_channels = 64, is_conv = True, is_norm = True, normalization = 'batch', is_act = True, activation = 'relu', 
            leak_param = 1/5.5):
...
def wide_resblk_1(inp_ten, stride = 2, in_channels = 16, widen_factor = 2, activation = 'relu', normalization = 'batch', leak_param = 1/5.5, name = None):   
...
#       Normalizing and applying activation on the input tensor 
...
#   Model architecture (1 conv -> 1 WR_blk_1 -> n WR_blk_2 -> 1 WR_blk_1 -> n WR_blk_2 -> 1 WR_blk_1 -> n WR_blk_2 -> Global_avg_pooling -> Dense) 
    ...
    with tf.variable_scope("Block_0"):
        ...
        for i in range(2): x = wide_resblk_2(x, name = "Block_" + str(num_blocks)); num_blocks += 1;
...
def lr_mom_calculator(num_epochs, max_lr, min_lr = None, train = False):  
    ...
    train:                    Boolean, whether model is running in train mode or Grid_SearchCV mode  
...
#   Decreasing the learning rate and increasing the momentum in the second part of the training
...
#   Decreasing the learning rate to several orders of magnitude and keeping the momentum constant in the last part of the training
...
def Lr_finder(epochs = 10, min_lr = 1e-5, max_lr = 100, params = ['Lr_finder'], fin_wt_dec = 0, fin_dropout = 0, vanilla = False, wt_dec_gridsearch = False, 
...
#   Get the learning rate collection corresponding to the first part of the training
    plt.figure(figsize = (21, 10)); epochs = epochs*2;
...
#               Get the iterators, LR collection and momentum collection
                ...
                learning_rate_collection = []; val_loss_collection = []; sess.run([tf.global_variables_initializer()]); 
                sess.run(train_iter.initializer); lr_coll, mom_coll = lr_mom_calculator(epochs, max_lr, min_lr, False);
...
#                   Run the training step to update the parameters and collect loss and acc values
                    _, train_loss, train_acc = sess.run([train_step, loss, accuracy], feed_dict = {X_ph: X_train_batch, 
                                               y_ph: y_train_batch, lr_ph: curr_lr, train_mode: True, beta1_ph: curr_beta1, dropout: drop_prob})
  
#                   Evaluate the trained model on eval set
                    if ptr % 75 == 0 and ptr > num_train_iters:
                        ...
                        for val_ptr in range(num_val_iters + 1):
                            ...
                            val_loss, val_acc = sess.run([loss, accuracy], feed_dict = {X_ph: X_val_batch, 
                                                          y_ph: y_val_batch, train_mode: False, dropout: 0});
                        
                            tot_val_loss += val_loss; tot_val_acc += val_acc;
				
                        if (tot_val_loss/num_val_iters + 1) < 15: 
                            val_loss_collection.append(tot_val_loss/(num_val_iters + 1)); learning_rate_collection.append(curr_lr); 
                        else: print("Max val_loss:  " + str(tot_val_loss/(num_val_iters + 1))); break;

                val_loss_collection_smooth = smooth(val_loss_collection); 
                plot_graph(learning_rate_collection, val_loss_collection_smooth, str(val), colors[ind]); 
...
tf.reset_default_graph(); Lr_finder(epochs = 10, max_lr = 3, min_lr = 1.5e-1, vanilla = True);

# Run the Grid_Search_CV for wt_dec values
tf.reset_default_graph(); wt_dec_list = [1e-6, 1e-5, 1e-4];
Lr_finder(epochs = 10, max_lr = 3, min_lr = 1.5e-1, params = wt_dec_list, wt_dec_gridsearch = True)

# Run the Grid_Search_CV for Dropout values
tf.reset_default_graph(); dropout_list = [0.15, 0.3, 0.45];
Lr_finder(epochs = 10, max_lr = 3, min_lr = 1.5e-1, params = dropout_list, fin_wt_dec = 1e-4, dropout_gridsearch = True)
...
#           Define the training step and iterators over the dataset
            ...
            handle, next_element, train_iter, train_handle, val_iter, val_handle, test_iter, test_handle = get_dataset_iterators(sess); 
            sess.run([tf.global_variables_initializer()]); lr_coll, mom_coll = lr_mom_calculator(num_epochs, 3, 1.5e-1, True); 
...
#               Evaluate the trained model on eval set if the condition is satisfied
                ...
                print(str(epoch) + ' Epochs:: Train_loss: ' + str(tot_train_loss/(num_train_iters + 1)) + ', Val_loss: ' + str(tot_val_loss/(num_val_iters + 1)) + ', Train_acc: ' + str((tot_train_acc/(num_train_iters + 1))*100) + '%, Val_acc: ' + str(tot_val_acc/(num_val_iters + 1)*100) + '%')
...
tf.reset_default_graph(); train(100);
