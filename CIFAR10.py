...
#   Randomly shuffling the training dataset, batching them and then prefetching them to make sure that a batch 
#   is ready to be served at all time   
...
#       Defining global variables that need to be passed within many functions 
        ...
        dropout = tf.placeholder(dtype = tf.float32, name = "Dropout"); beta1_ph = tf.placeholder(shape = [], dtype = tf.float32);
        lr_ph = tf.placeholder(dtype = tf.float32, name = "Learning_rate"); train_mode = tf.placeholder(dtype = tf.bool, name = 'Mode');  
...
def Lr_finder(epochs = 10, min_lr = 1e-5, max_lr = 100, params = ['Lr_finder'], fin_wt_dec = 0, fin_dropout = 0, vanilla = False, 
...
#               Evaluate the trained model on eval set if the condition is satisfied
                ...
                print(str(epoch) + ' Epochs:: Train_loss: ' + str(tot_train_loss/(num_train_iters + 1)) + ', Val_loss: ' + str(tot_val_loss/(num_val_iters + 1)) + \
