...
#   Randomly shuffle the training dataset, make batches and prefetch one of them 
...
#   Apply Normalization
    if is_norm:
        ...
        elif(normalization == 'batch'): x = tf.layers.batch_normalization(x, epsilon = 1e-5, training = train_mode, momentum = 0.85)
...
#                   Run the training step to update the parameters
                    _, train_loss, train_acc = sess.run([train_step, loss, acc], feed_dict = {X_ph: X_train_batch, y_ph: y_train_batch, train_mode: True, dropout: 0.3, 
